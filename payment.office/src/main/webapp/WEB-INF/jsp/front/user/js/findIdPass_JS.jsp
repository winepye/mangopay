...
$(document).ready(function(){
    ...
    //탭 디폴트 활성화 :아이디
    fn_moveTab("${userCriteria.searchFindType==null?'id':userCriteria.searchFindType}");
    //핸드폰번호
...
function fn_moveTab(type){
    ...
    case 'password' :
        ...
        $("#tabPassword").addClass("on");
        $("#tabPassword").append("<em id=\"emId\" class=\"point\"></em>");
        $("#tabId emId").remove();
        $("#userNameArea").hide();
        $("#userIdArea").show();
        break;
    }
    $("#searchFindType").val(type);
...
function fn_reqConfirmNoCallback(data){
	 //휴면계정이고 비밀번호 찾기인경우는 휴면계정 복원페이지로 바로 이동함
    if($("#searchFindType").val() == 'password' && data.userStatusCd == "<c:out value='${Constants.USER_STATUS_CD_DORMANT }' />"){
...
function fn_processResultStep(){
    ...
    if(type == 'password'){
        ...
        $("#frm").attr("action" , contextRoot + "/user/findIdPassword/getConfirmNumberAgreeAjax.do");
...
function fn_goConfirmNumberAgreeCallBack(data){
    if(data.result == "<c:out value='${Constants.RESULT_SUCCESS }' />"){
        $(".joinList").hide();
        $("#idTxt").html("'"+data.userId+"'"); /* 회원의 아이디 세팅 */
        $(".idResult").show();
    }
    else{
    	alert("<spring:message code='noti.myPage.noUserInfo' />");  //일치하는 회원정보가 없습니다.
