...
$(document).ready(function(){
  ...
  datePickerWrap("#equipDateS");
  datePickerWrap("#equipDateE");
  fn_setDate1();
...
//현재 날짜 셋팅
function fn_setDate1(){
  var now = new Date();
  var searchStartDt = now.yyyy_mm_dd();
  $("#equipDateS").val(searchStartDt);
  $("#equipDateE").val(searchStartDt);
}
...
function fn_doSearch(pageNo){
	$("#frm").attr("action" , contextRoot + "/admin/company/getCompanyListAjax.do");
...
//상세화면으로 이동
function fn_goDetailPage(brandInfoId, companyInfoId){
	$("#brandInfoId").val(brandInfoId);
	$("#companyInfoId").val(companyInfoId);

	$("#selectedBrandInfoId").val(brandInfoId);
...
function fn_doSort(type){
	...
	if(type == 1){
		...
		if(targetClass == "sort_hightolow"){
			...
			$("#sortOrder").val("A.BRAND_INFO_ID ASC");
			...
			$("#sortOrder").val("A.BRAND_INFO_ID DESC");
		}
	}else if(type == 2){
...
//등록페이지이동
function goBrandCreatePage(){
	$("#frm").attr("action",contextRoot + "/admin/brand/brandCreate.do");
	$("#frm").submit();
}
