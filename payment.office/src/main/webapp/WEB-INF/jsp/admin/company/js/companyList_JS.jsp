...
//상세화면으로 이동
function fn_goDetailPage(companyInfoId){
	$("#companyInfoId").val(companyInfoId);
	$("#selectedCompanyInfoId").val(companyInfoId);
	$("#frm").attr("action",contextRoot + "/admin/company/companyDetail.do");
	$("#frm").submit();
}
...
function fn_doSort(type){
	...
	if(type == 1){
		...
		if(targetClass == "sort_hightolow"){
			...
			$("#sortOrder").val("A.COMPANY_INFO_ID ASC");
			...
			$("#sortOrder").val("A.COMPANY_INFO_ID DESC");
		}
	}else if(type == 2){
...
//등록페이지이동
function goCompanyCreatePage(){
	$("#frm").attr("action",contextRoot + "/admin/company/companyCreate.do");
	$("#frm").submit();
}
