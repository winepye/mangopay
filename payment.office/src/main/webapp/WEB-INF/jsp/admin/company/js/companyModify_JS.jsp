...
$(document).ready(function(){
    ...
    //수정버튼
    $("#btnModifyCompany").click(function(){
    	fn_doModifyCompany();
    });
...
function fn_doModifyCompany(){
   ...
   /*  if($("#corpNum1").val() == ''){
    ...
    if(confirm("<spring:message code='request.common.update'   />")) {
        $("#frm").attr("action",contextRoot + "/admin/company/modifyCompanyAjax.do");
        commonAjaxSubmit("POST", $("#frm"), fn_doModifyCompanyCallback);
    }
...
//저장 콜백
function fn_doModifyCompanyCallback(data){
	if(data.result == "<c:out value='${Constants.RESULT_SUCCESS }' />"){
		alert("정상적으로 저장되었습니다.");
		fn_goCompanyDetailPage();
	}else{
...
//상세 페이지로 이동
function fn_goCompanyDetailPage(){
