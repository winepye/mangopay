...
$(document).ready(function(){
    ...
    //등록버튼
    $("#btnCreateCharger").click(function(){
        fn_doCreateCharger();
    });
...
function fn_doCreateCharger(){
	...
	$("#frm").attr("action",contextRoot + "/admin/charger/createChargerAjax.do");
	commonAjaxSubmit("POST", $("#frm"), fn_doCreateChargerCallback);
}
//저장 콜백
function fn_doCreateChargerCallback(data){
	...
	if($("#companyInfoId").val() != "0"){
		...
		<c:forEach var="item" items="${brandInfoList}">
			...
			if('${item.companyInfoId}' == $("#companyInfoId").val()){
				var option = $("<option value=\"${item.brandInfoId}\">${item.brandName}</option>");
	...
	if($("#brandInfoId").val() != "0"){
		...
		<c:forEach var="item" items="${shopInfoList}">
			...
			if('${item.brandInfoId}' == $("#brandInfoId").val()){
				var option = $("<option value=\"${item.shopInfoId}\">${item.shopName}</option>");
