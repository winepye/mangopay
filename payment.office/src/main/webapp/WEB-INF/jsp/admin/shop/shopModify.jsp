...
<!--
  * @Class Name  : payModify.jsp
  * @Description : 결제서비스 수정화면
...
<section id="detail_contents">
	<form id = "frm" name = "frm" method="post">
		<h3 class="h2_table_title">결제서비스 수정</h3>
       	<table id="templateProject" class="tb_left_st2" summary="결제서비스 수정">
                   ...
                   <td>
                       <select name="serviceProvider" id="serviceProvider" class="sel_basic w100pro">
                           ...
                           <c:forEach var="item" items="${serProvList}">
                               <option value="${item.cd}" <c:if test="${item.cd eq result.serviceProvider}">selected="selected"</c:if> >${item.cdNm}</option>
                           </c:forEach>
                       </select>
                   </td>
                   ...
                   <td>
                       <select name="paymentMean" id="paymentMean" class="sel_basic w100pro">
                           ...
                           <c:forEach var="item" items="${payMeanList}">
                               <option value="${item.cd}" <c:if test="${item.cd eq result.paymentMean}">selected="selected"</c:if> >${item.cdNm}</option>
                           </c:forEach>
                       </select>
                   </td>
                   ...
                   <td>
                       <select name="paymentMethod" id="paymentMethod" class="sel_basic w100pro">
                           ...
                           <c:forEach var="item" items="${payMethodList}">
                               <option value="${item.cd}" <c:if test="${item.cd eq result.paymentMethod}">selected="selected"</c:if> >${item.cdNm}</option>
                           </c:forEach>
                       </select>
                   </td>
                   <th scope="row">담당자</th>
		...
		<div class="bs_BtnBox_c">
            ...
            <button type="button" id="btnGoPayPreviousPage" class="btn_white_B01" onclick="javascript:history.back();">취소</button>
            <button type="button" id="btnModifyPay" class="btn_gray_B01">수정완료</button>
        </div>
		<!-- 버튼 -->
...
<%@ include file="js/shopModify_JS.jsp"%>