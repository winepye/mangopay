...
function fn_doInit(){
    ...
    $("input:radio[name='searchQnaStatusCd']:radio[value='']").prop("checked", true);
}

//작성 페이지로 이동
function fn_goQnaCreatePage(){
	$("#frm").attr("action",contextRoot + "/admin/board/qna/qnaCreate.do");
	$("#frm").submit();
}

//검색 ,페이지 이동
function fn_doSearch(pageNo){
	$("#frm").attr("action" , contextRoot + "/admin/board/qna/getQnaBoardListAjax.do");
...
function fn_deleteQna(){
	...
	$("#frm").attr("action" , contextRoot + "/admin/board/qna/deleteQnaAjax.do");
...
function fn_goDetailPage(boardSeq){
	...
	$("#frm").attr("action",contextRoot + "/admin/board/qna/qnaDetail.do");
	$("#frm").submit();
}
