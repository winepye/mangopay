...
<section id="detail_contents">
	<form id = "frm" name = "frm" method="post">
        ...
        <div class="adm_searchBox">
               <table id="reg_input_grid" class="adm_tb_search" summary="공지사항검색 (제목/내용, 게시여부, 작성일)">
                   ...
                   <tbody>
                       <tr>
                           ...
                           <td class="vtwrap">
				                <select id="searchContentType" name = "searchContentType" class="sel_basic w100px" title="공지사항검색">
                                ...
                                <input id="searchContentText" name = "searchContentText" type="text"
                                   ...
                                   class="input_s_b w240px mgl5" title="공지사항검색" value="${boardCriteria.searchContentText }" autocomplete="off" maxlength="20"/>
                           </td>
                           <th scope="row"><label for="searchOpenYnAll">게시여부</label></th>
                           <td class="itemList">
                               <label for="searchOpenYnAll">
	                               <input type="radio" id="searchOpenYnAll" name="searchOpenYn" <c:if test="${boardCriteria.searchOpenYn eq '' || boardCriteria.searchOpenYn eq null}"> checked="checked" </c:if> value="" style="border: none;">
	                               <span>전체</span>
                               </label>
                               <customTagUi:code type="radio" cdGrp="YN_COMMON" name="searchOpenYn" id="searchOpenYn" cd="${boardCriteria.searchOpenYn }" prefix="<span>" postfix="</span>"/>
                           </td>
                       </tr>
                       <tr>
                           <th scope="row"><label for="searchFromDtS">작성일</label></th>
                           <td colspan="3" class="vtwrap">
                               <input type="text" style="width: 80px" class="input_s_b w_date" id = "searchFromDtS" name = "searchFromDtS" readonly="readonly" value="<fmt:formatDate value="${boardCriteria.searchFromDt}" pattern="yyyy/MM/dd"/>"/>
                               <span>~</span>
                               <input type="text" style="width: 80px" class="input_s_b w_date" id = "searchToDtS" name="searchToDtS" readonly="readonly" value="<fmt:formatDate value="${boardCriteria.searchToDt}" pattern="yyyy/MM/dd"/>"/>
