...
function fn_goNotiCreatePage(){
	$("#frm").attr("action",contextRoot + "/admin/board/noti/notiCreate.do");
	$("#frm").submit();
}

//검색 ,페이지 이동
function fn_doSearch(pageNo){
	$("#frm").attr("action" , contextRoot + "/admin/board/noti/getNotiListAjax.do");
...
function fn_deleteNoti(){
	...
	$("#frm").attr("action" , contextRoot + "/admin/board/noti/deleteNotiAjax.do");
	commonAjaxSubmit("POST", $("#frm"), fn_deleteNotiCallback);
}
//선택 삭제 콜백
function fn_deleteNotiCallback(data){
...
function fn_goDetailPage(boardSeq){
	...
	$("#frm").attr("action",contextRoot + "/admin/board/noti/notiDetail.do");
	$("#frm").submit();
}
